...
`state.json` is authoritative for structured status. `CURRENT_STATE.md` must mirror its labeled metadata, task table, and exact ordered blocker list. Every test-evidence entry requires `command`, `result`, `outcome` (`passed` or `failed`), and `verified_at`; review or final approval requires valid passed evidence. Run `python scripts/check_project_memory.py` after every memory update.
